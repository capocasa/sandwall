...
when defined(windows):
  ...
  proc spawnSandboxed*(cmd: openArray[string]): ExitCode =
    ...
    if currentRestrictedToken == 0:
      raise newException(OSError, "procbox: restrict() must be called first")
    ...
    if cmdLine.len == 0:
      raise newException(ValueError, "procbox.spawnSandboxed: empty command")
    ...
    if createProcessAsUserW(currentRestrictedToken, nil,
      ...
      raise newException(OSError,
        "procbox: CreateProcessAsUserW failed: " & $getLastError())
    defer:
    ...
    if w == WAIT_FAILED:
      raise newException(OSError,
        "procbox: WaitForSingleObject failed: " & $getLastError())
    var code: int32 = 0
    if getExitCodeProcess(pi.hProcess, code) == 0:
      raise newException(OSError,
        "procbox: GetExitCodeProcess failed: " & $getLastError())
  ...
  proc forkNimbox*(): Pid =
    ...
    result = posix.fork()
    if result < 0:
      raise newException(OSError, "procbox: fork() failed: " & osLastError().`$`)
  ...
  proc exec*(cmd: openArray[string]) =
    ...
    if cmd.len == 0:
      raise newException(ValueError, "procbox.exec: empty command")
    let prog0 = cmd[0]
    ...
    raise newException(OSError,
      "procbox: exec(" & cmd.join(" ") & ") failed: " & osLastError().`$`)
  ...
  proc wait*(pid: Pid): ExitCode =
    ...
    if posix.waitpid(pid, status, 0) < 0:
      raise newException(OSError,
        "procbox: waitpid failed: " & osLastError().`$`)
    if WIFEXITED(status):
